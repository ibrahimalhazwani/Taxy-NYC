...
```{r}
yellow_df_sample <- yellow_df[sample(nrow(yellow_df), 600), ] %>% 
  subset(select = c(PULocationID,DOLocationID)) %>%
  drop_na()

yellow_nyc_PUL <- right_join(yellow_df_sample,map_nyc,"PULocationID") %>%
...
# aggiungere un suffisso per distignure target e source
...
nodes2 <- data.frame(name=c(as.character(data_long$source), as.character(data_long$target)) %>% unique())

data_long$IDsource=match(data_long$source, nodes2$name)-1 
data_long$IDtarget=match(data_long$target, nodes2$name)-1
