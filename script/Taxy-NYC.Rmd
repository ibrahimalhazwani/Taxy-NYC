...
```{r}
PULocation <- unique(as.character(yellow_df_sample$PULocationID))
DOLocation <- unique(as.character(yellow_df_sample$DOLocationID))
values <- group_by(yellow_df_sample, PULocationID) %>%
                       count()

sq <- seq(min(length(PULocation), length(DOLocation)))
links <- data.frame(source = PULocation[sq],
                    target = DOLocation[sq],
                    value = values)
