...
Plot the NYC map
```{r}
yellow_df_sample_taxy_map <- yellow_nyc_PUL %>% group_by(PULocationID) %>%
  summarize(count=n())

yellow_df_sample_taxy_map <- left_join(yellow_df_sample_taxy_map,map_nyc, 'PULocationID') %>%
  subset(select = c(PULocationID, count, geometry))

ggplot() +
  geom_sf (data = map_nyc, size = 0.03) +
  geom_sf (data = yellow_df_sample_taxy_map, mapping = aes(fill = count, geometry = geometry), size = 0.05) + 
